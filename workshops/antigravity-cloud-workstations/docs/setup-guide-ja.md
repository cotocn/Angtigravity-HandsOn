...
|    └─ Workstation VM (Private VPC: 10.10.0.0/20, Zero Public IP)                  |
...
| レイヤ | リソース / 設計 | 説明 |
|---|---|---|
| **ネットワーク** | カスタム Private VPC + Cloud NAT + Private Google Access | VM にパブリック IP を一切持たせず（ゼロパブリック IP）、Cloud NAT 経由で安全にアウトバウンド通信 |
...
| **端末の要件** | ・`gcloud` CLI (最新)<br>・`terraform` (v1.6+) または Google Cloud Shell | **Web ブラウザ（Google Chrome 推奨）のみ**<br>※PC 端末への Git、VS Code、CLI、拡張機能のインストールは一切不要 |
| **必要な IAM 権限** | ・`roles/workstations.admin`（クラスター・構成・インスタンス管理）<br>・`roles/iam.serviceAccountAdmin`（サービスアカウント作成）<br>・`roles/compute.networkAdmin`（VPC・サブネット管理）<br>・`roles/artifactregistry.admin`（カスタムイメージ用） | ・**`roles/workstations.user`**（自分専用の Workstation へのアクセス権）<br>・**`roles/viewer`**（GCP コンソールで一覧画面を表示する場合。直接 URL を開く運用の場合は不要）<br>※他人の環境の閲覧やインフラ設定の変更はできません |
...
## 4.2. Google Cloud コンソール（GUI）で作成する場合の手順【管理者のみ実施】

Terraform を使わず、Google Cloud コンソール画面（Web GUI）からマウス操作のみでセットアップする手順です。
以下の **4 つのステップ** の順序で作成します。

```
[ステップ 1: サービスアカウント作成]
         │
         ▼
[ステップ 2: ワークステーション クラスター作成]（約10〜15分）
         │
         ▼
[ステップ 3: ワークステーション 構成（Config）作成]（マシン・セキュリティ設定）
         │
         ▼
[ステップ 4: ワークステーション（インスタンス）作成]（受講者ごとの環境）
```

---

### ステップ 1: サービスアカウントの作成（API キー不要化）
...
### ステップ 2: ワークステーション クラスターの作成
Workstations をホストするネットワーク基盤を作成します。
...
3. 以下の項目を設定します：
   ...
   - **ネットワーク**: 
     - 既存の社内 VPC を使う場合: **既存の VPC ネットワークとサブネット** を選択
     - 新規作成の場合: 事前に作成した専用 VPC を選択
...
3. **基本情報**:
   ...
   - **クラスター**: ステップ 2 で作成した `antigravity-cluster` を選択
4. **マシンの構成**:
...
7. **詳細設定 (セキュリティ & ID)**:
   - **サービス アカウント**: ステップ 1 で作成した `antigravity-ws-sa` を選択
   - **パブリック IP アドレスを無効にする**: **チェックを入れる**（ゼロパブリック IP 化）
8. **[作成]** をクリックします。

---

### ステップ 4: ワークステーションの作成 & 起動
受講者が実際に作業する個別インスタンスを作成します。

1. コンソールで **[Cloud Workstations]** > **[ワークステーション]** を開きます。
2. 上部の **[+ ワークステーションを作成]** をクリックします。
3. 以下の項目を設定します：
   - **ワークステーション名**: `participant-workstation`（または受講者名 `user01-ws` 等）
   - **構成**: ステップ 3 で作成した `antigravity-config` を選択
...
## 5. コンテナ定義とビルド（Cloud Build）【※通常はスキップ可能・アドバンスド】
...
## 6. 接続方法 & 運用モデル（管理者と受講者の役割分担）

### 6.1. 運用モデルの全体像：「管理者が土台を用意し、受講者はコンテナを起動するだけ」
...
### 6.2. 複数ワークステーションの事前プロビジョニング【管理者のみ実施】
...
### 6.3. 受講者の当日接続ステップ（これだけ案内すればOK）【受講者が実施】
...
### 6.4. 各受講者による接続方法（2 通りの選択肢）【受講者が実施】
